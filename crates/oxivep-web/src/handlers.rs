...
/// Scans for .gff3, .gff3.gz, and .oxivep.cache files.
        ...
        for entry in entries.flatten() {
            ...
            if path.is_dir() {
                let name = path
                    ...
                    .unwrap_or_default();
                let gff3 = find_file_with_ext(&path, &["gff3", "gff3.gz", "oxivep.cache"]);
        ...
        for entry in entries.flatten() {
            ...
            if path.is_file() {
                ...
                if fname.ends_with(".gff3")
                    ...
                    || fname.ends_with(".oxivep.cache")
                {
                    let stem = fname
                        .trim_end_matches(".oxivep.cache")
                        .trim_end_matches(".gz")
...
fn resolve_genome_paths(
    ...
    if subdir.is_dir() {
        let gff3 = find_file_with_ext(&subdir, &["gff3", "gff3.gz", "oxivep.cache"])
    ...
    for ext in &["gff3", "gff3.gz", "oxivep.cache"] {
