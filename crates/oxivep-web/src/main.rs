...
#[derive(Parser)]
#[command(name = "oxivep-web")]
#[command(about = "OxiVEP production web server for variant annotation")]
struct Cli {
    /// Port to listen on
    #[arg(long, default_value_t = 8080, env = "OXIVEP_PORT")]
    port: u16,

    /// Bind address
    #[arg(long, default_value = "0.0.0.0", env = "OXIVEP_BIND")]
    bind: String,

    /// GFF3 annotation file to load at startup
    #[arg(long, env = "OXIVEP_GFF3")]
    gff3: Option<String>,

    /// FASTA reference file (requires .fai index for mmap)
    #[arg(long, env = "OXIVEP_FASTA")]
    fasta: Option<String>,

    /// Supplementary annotation directory (.osa/.osa2 files)
    #[arg(long, env = "OXIVEP_SA_DIR")]
    sa_dir: Option<String>,

    /// Directory containing genome data (subdirs with GFF3 + FASTA per organism)
    #[arg(long, env = "OXIVEP_DATA_DIR")]
...
async fn main() -> anyhow::Result<()> {
    tracing_subscriber::fmt()
        .with_env_filter(
            tracing_subscriber::EnvFilter::try_from_default_env()
                .unwrap_or_else(|_| "oxivep_web=info".into()),
        )
        .init();
    ...
    tracing::info!("OxiVEP web server listening on http://{}", addr);
