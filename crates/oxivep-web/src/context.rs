...
impl AnnotationContext {
    ...
    pub fn new(
        ...
        let mut transcripts = if let Some(gff3_path) = gff3 {
            let cache_path =
                oxivep_cache::transcript_cache::default_cache_path(Path::new(gff3_path));
            let from_cache = if cache_path.exists() {
                let is_fresh = oxivep_cache::transcript_cache::cache_is_fresh(
                    &cache_path,
                    Path::new(gff3_path),
                );
                if is_fresh {
        ...
        let seq_provider: Option<Box<dyn SequenceProvider + Send + Sync>> =
            if let Some(fasta_path) = fasta {
                ...
                if Path::new(&fai_path).exists() {
                    let reader =
                        oxivep_cache::fasta::MmapFastaReader::open(Path::new(fasta_path))?;
                    tracing::info!("Memory-mapped FASTA from {}", fasta_path);
                    Some(Box::new(
                        oxivep_cache::providers::MmapFastaSequenceProvider::new(reader),
                    ))
                } else {
                                ...
                                if let Some(tr) = transcript {
                                        ...
                                        (ac.cdna_start, ac.cdna_end)
                                    {
                                            ...
                                            tr.genomic_to_intronic_cdna(vf.position.start)
                                        {
                                            ...
                                            ann.hgvsc =
                                                oxivep_hgvs::hgvsc_noncoding_intronic_range(
                                                    &versioned_tid,
                                                    cdna_pos,
                                                    offset,
                                                    end_cdna,
